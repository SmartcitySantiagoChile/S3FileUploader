...
from get_objects_values_from_s3 import main as get_object_values_main
...
class DeleteObjectTest(TestCase):
    ...
    def test_bucket_is_deleted(self, aws_session_mock):
        ...
        expected_answer = (
            "INFO:delete_object_in_s3:Object aaa.txt was deleted successfully!"
        )
        self.assertIn(expected_answer, f.output)

        aws_session_mock.return_value.delete_object_in_bucket.assert_called_once()
        aws_session_mock.return_value.delete_object_in_bucket.assert_called_with(
    ...
    def test_bucket_raise_error_when_is_deleting(self, aws_session_mock):
        ...
        aws_session_mock.return_value.check_bucket_exists.return_value = True
        aws_session_mock.return_value.delete_object_in_bucket.side_effect = ClientError(
...
class UploadObjectTest(TestCase):
    ...
    def test_move_file_to_bucket_with_wrong_filename_and_omit_filename_check_param(
        ...
        with self.assertLogs("upload_to_s3", level="INFO") as f:
            upload_main(
                [self.command_name, filepath, bucket_name, "--omit-filename-check"]
            )
    ...
    def test_move_file_to_bucket_with_client_error(self, glob_mock, aws_session_mock):
        ...
        with self.assertLogs("upload_to_s3", level="ERROR") as f:
            upload_main([self.command_name, filepath, bucket_name])
            expected_answer = (
                f"An error occurred ({error_response['Error']['Code']}) when calling the delete "
...
class DownloadObjectTest(TestCase):
    ...
    def test_bucket_raise_error_when_is_downloading(self, aws_session_mock):
        ...
        error_response = dict(Error=dict(Code=403, Message="forbidden"))
        aws_session_mock.return_value.check_bucket_exists.return_value = True
        aws_session_mock.return_value.download_object_from_bucket.side_effect = (
...
class DeleteBucketTest(TestCase):
    ...
    @mock.patch("delete_bucket_from_s3.AWSSession.check_bucket_exists")
    def test_bucket_raise_error_when_is_deleting(
        self, check_bucket_exists, delete_bucket
    ):
        ...
        expected_answer = (
            "ERROR:delete_bucket_from_s3:An error occurred (403) when calling the deleting operation: "
            "forbidden"
        )
        self.assertIn(expected_answer, f.output)
...
class MoveBucketTest(TestCase):
    ...
    def test_bucket_raise_error_when_is_moving(
        ...
        expected_answer = (
            "ERROR:move_bucket_from_s3:An error occurred (403) when calling the moving operation: "
            "forbidden"
        )
        self.assertIn(expected_answer, f.output)
...
class UpdateObjectsFromS3Test(TestCase):
    ...
    @mock.patch("move_bucket_from_s3.AWSSession.check_bucket_exists")
    def test_with_bucket_does_not_exist(
        self, check_bucket_exist, update_files_from_bucket
    ):
...
class TestGetObjectValuesFromS3(TestCase):
    def setUp(self):
        self.command_name = "get_object_values_from_s3"

    def test_without_params(self):
        with self.assertRaises(SystemExit):
            get_object_values_main([self.command_name])

    def test_with_one_param(self):
        source_bucket = "source"
        with self.assertRaises(SystemExit):
            get_object_values_main([self.command_name, source_bucket])

    def test_with_two_params(self):
        extension = ".bip"
        source_bucket = "source"
        with self.assertRaises(SystemExit):
            get_object_values_main([self.command_name, source_bucket, extension])
    ...
    def test_with_three_params(self):
        ...
        tuples = "[1,2,3]"
        with self.assertRaises(SystemExit):
            get_object_values_main(
                [self.command_name, source_bucket, extension, tuples]
            )
    ...
    @mock.patch("aws.AWSSession.retrieve_values_from_bucket_files")
    @mock.patch("aws.AWSSession.check_bucket_exists")
    def test_with_bucket_does_not_exist(
        self, check_bucket_exist, retrieve_values_from_bucket_files
    ):
        check_bucket_exist.return_value = True
        source_bucket = "source"
        tuples = "[1,2,eq,4]"
        ...
        start_date = "2022-10-01"
        end_date = "2022-10-01"
        retrieve_values_from_bucket_files.return_value = None
        self.assertIsNone(
            get_object_values_main(
                [
                    self.command_name,
