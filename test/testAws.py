...
class AwsTest(TestCase):
    ...
    def test_check_bucket_exists_true(self):
        bucket = mock.MagicMock(
            meta=mock.MagicMock(
                client=mock.MagicMock(head_bucket=mock.MagicMock(return_value=True))
            )
        )
        self.aws_session.session.resource = mock.MagicMock(return_value=bucket)
    ...
    def test_check_bucket_exists_404_error(self):
        ...
        error.side_effect = ClientError(
            error_response=error_response, operation_name="mock error"
        )
        bucket = mock.MagicMock(
    ...
    def test_check_file_exists_404_error(self):
        error = mock.MagicMock()
        error_response = {"Error": {"Code": "404"}}
        error.load.side_effect = ClientError(
            error_response=error_response, operation_name="mock error"
        )
        bucket = mock.MagicMock()
    ...
    def test_send_file_to_bucket(self, build_url):
        ...
        self.assertEqual(
            "url", self.aws_session.send_file_to_bucket("path", "key", "bucket_name")
        )
    ...
    def test_send_object_to_bucket(self, build_url):
        ...
        self.aws_session.session.resource = mock.MagicMock()
        self.assertEqual(
            "url", self.aws_session.send_object_to_bucket("obj", "key", "bucket_name")
        )
    ...
    def test_delete_object_in_bucket(self):
        ...
        self.aws_session.session.resource = mock.MagicMock(return_value=bucket)
        self.assertEqual(
            "delete", self.aws_session.delete_object_in_bucket("key", "bucket_name")
        )
    ...
    def test_copy_files_from_bucket_to_bucket(self):
        ...
        self.aws_session.copy_file_from_bucket_to_bucket(
            "source_bucket", "target_bucket", "file_name"
        )
    ...
    def test_move_files_from_bucket_to_bucket(self):
        ...
        self.aws_session.delete_object_in_bucket = mock.MagicMock()
        self.aws_session.move_files_from_bucket_to_bucket(
            source_bucket_name, target_bucket_name, datafiles, extension_list
        )
        # one file case
    ...
    def test_update_files_from_bucket(
        self, retrieve_objects_with_pattern, get_file_object
    ):
        date_list: list = [
        ...
        self.aws_session.update_files_from_bucket(
            date_list,
            bucket_name,
            extension,
            tuples_list,
    ...
    @mock.patch("aws.get_file_object")
    @mock.patch("aws.retrieve_objects_with_pattern")
    def test_retrieve_values_from_bucket_files(
        self, retrieve_objects_with_pattern, get_file_object
    ):
        bucket_name: str = "adatrap-bip123"
        date_list: list = [
        ...
        self.aws_session.retrieve_obj_list = mock.MagicMock(return_value=obj_list)
        self.aws_session.download_object_from_bucket = mock.MagicMock()
        self.aws_session.retrieve_values_from_bucket_files(
